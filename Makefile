...
EXTENSION_NAME=PARALLEL_PYTHON
EXTENSION_FLAGS=\
-DDUCKDB_EXTENSION_NAMES="parallel_python" \
...
debug:
	mkdir -p  build/debug && \
	cmake $(GENERATOR) $(BUILD_FLAGS) $(CLIENT_FLAGS) -DENABLE_UBSAN=0 -DCMAKE_BUILD_TYPE=Debug -S ./duckdb/ -B build/debug && \
...
#### Client tests
DEBUG_EXT_PATH='$(PROJ_DIR)build/debug/extension/custom_dlopen/custom_dlopen.duckdb_extension'
RELEASE_EXT_PATH='$(PROJ_DIR)build/release/extension/custom_dlopen/custom_dlopen.duckdb_extension'
